...
```{r}
dat_ov <- read_sheet('https://docs.google.com/spreadsheets/d/1yCTnICWn2zVF5pQH5ARNhXM3eCZ1DsY1htO202Khowk/edit#gid=0', sheet = "1. Overview&24h")[,1:7] %>%
  clean_names()
```
...
```{r}
dat_area <- read_sheet('https://docs.google.com/spreadsheets/d/1yCTnICWn2zVF5pQH5ARNhXM3eCZ1DsY1htO202Khowk/edit#gid=0', sheet = "4. Area") %>% 
  clean_names()
```
...
```{r}
dat_counts <- read_sheet('https://docs.google.com/spreadsheets/d/1yCTnICWn2zVF5pQH5ARNhXM3eCZ1DsY1htO202Khowk/edit#gid=0', sheet = "3. Counts") %>% 
  clean_names()
```
...
```{r}
dat_chl24h_ov <- read_sheet('https://docs.google.com/spreadsheets/d/1yCTnICWn2zVF5pQH5ARNhXM3eCZ1DsY1htO202Khowk/edit#gid=0', sheet = "1. Overview&24h") %>% 
  clean_names()

dat_chl48h_ov <- read_sheet('https://docs.google.com/spreadsheets/d/1yCTnICWn2zVF5pQH5ARNhXM3eCZ1DsY1htO202Khowk/edit#gid=0', sheet = "2. 48 h") %>% 
  clean_names()
```
...
```{r}
dat_chl <- read_all_files(here('physio/data/raw_data/before_bleaching'))
...
Except for one measurement, the 48 h doesn't add much.

Find out which one, print top 5

```{r}
dat_chl %>% 
  filter(extr_time == "48 h") %>% 
  arrange(-chl_a_per_cm2) %>% 
  slice(1:5)
```

See both measurements

```{r}
dat_chl %>% 
  filter(extr_time == "48 h", sample_id == "TSAS72")
```
...
```{r}
dat_ov_all <- read.csv(here("overview/data/2024_04_02_charlottes_samples.csv"))
```

In this file, **`B`** is used for `t` to indicate that these samples (only 8 AL) were sampled *before* the actual t3 sampling. Adapt the `sample_id` in `dat_all` accordingly:

```{r}
dat_all <- dat_all %>% 
  mutate(sample_id = ifelse(nchar(sample_id) == 6, paste0("B", sample_id, "P"),sample_id))
```
...
```{r}
write.csv(dat_all, file = here("physio/data/clean/before_bleaching.csv"))
```
