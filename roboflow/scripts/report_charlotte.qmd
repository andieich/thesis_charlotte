...
```{r}
plot_bleaching_state <- d_pigm_b_stat_AL %>% 
  ...
  geom_point(position = position_jitter(.2), shape = 21)+
  facet_wrap(~Parameter, scales = "free", , labeller = label_parsed)+
  labs(y = "", x = expression("Chlorophyll a (µg "*cm^2*")"))+
  scale_fill_manual(values = c("skyblue", "darkorange"), name = "")+
  theme_minimal()+
  theme(legend.position = "bottom")
...
```{r}
ggsave(filename = "plot_bleaching_state.png", plot = plot_bleaching_state, path = here("roboflow/plots"),width = 20, height =12,units = "cm", scale = 0.7)
```
